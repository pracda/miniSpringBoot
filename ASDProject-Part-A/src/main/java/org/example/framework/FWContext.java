...
public class FWContext {
    ...
    public FWContext() {
        try {
            Reflections reflections = new Reflections("org.example.application");
            Set<Class<?>> serviceClasses = reflections.getTypesAnnotatedWith(Service.class);
            for (Class<?> serviceClass : serviceClasses) {
                serviceClassObject.add(serviceClass.newInstance());
            }
    ...
    public Object getServiceBeanOfType(Class<?> instanceClass) {
        ...
        try {
            for (Object theClass : serviceClassObject) {
                ...
                if (clazz.getName().equals(instanceClass.getName())) {
                    service = theClass;
                }
            }
        } catch (Exception e) {
